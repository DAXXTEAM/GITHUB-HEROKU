...
def get_last_commit_date(owner, repo, branch, file_path):
    query = f""
        {{
    ...
    try:
        commit_dates = [datetime.strptime(range["commit"]["committedDate"], "%Y-%m-%dT%H:%M:%SZ") for range in data["data"]["repository"]["object"]["blame"]["ranges"]]
...
def check_github_for_updates():
    ...
    for repo in all_repositories:
        repo_name = repo["name"]
        local_repo_path = f"/path/to/local/repositories/{repo_name}"
        ...
        latest_commit_sha = subprocess.run(["git", "rev-parse", "HEAD"], cwd=local_repo_path, capture_output=True, text=True).stdout.strip()
        if latest_commit_sha != repo["default_branch"]:
            file_path = f"{local_repo_path}"
            branch = repo["default_branch"]
            last_commit_date = get_last_commit_date(owner, repo_name, branch, file_path)

            if last_commit_date:
                local_commit_date = datetime.fromisoformat(repo["pushed_at"].replace("Z", "+00:00"))
                if last_commit_date > local_commit_date:
                    send_notification(f"New commit detected in {repo_name}. Updating local repository...")
                    perform_git_pull(local_repo_path)
